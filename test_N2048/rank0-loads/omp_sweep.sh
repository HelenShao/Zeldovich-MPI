...
THREADS=(1 2 4 8 16 32 51)
PARAM_SOURCE="$SCRIPT_DIR/param_N2048_CPD_225.par"

source_pbs() {
    case $1 in
        1) echo "$SCRIPT_DIR/N2048_CPD225_MPI32_OMP1.pbs" ;;
        *) echo "$SCRIPT_DIR/omp-$1/N2048_CPD225_MPI32_OMP${1}.pbs" ;;
    esac
}
...
source_reldir() {
    case $1 in
        1) echo "test_N2048/MPI-32-nodes" ;;
        *) echo "test_N2048/MPI-32-nodes/omp-$1" ;;
    esac
}
...
cmd_analyze() {
    ...
    # Prefer per-experiment plotting script when present (created by setup).
    # Fall back to the shared script in MPI-32-nodes for pre-existing directories.
    if [[ -f "$dir/scripts/plot_omp_stage_scaling.py" ]]; then
        plot_script="$dir/scripts/plot_omp_stage_scaling.py"
    else
        plot_script="$SCRIPT_DIR/plot_omp_stage_scaling.py"
    fi
    [[ -f "$plot_script" ]] || {
    ...
    # 1) setup-generated: DIR/runs/omp-*/...
    # 2) legacy/manual:   DIR/omp-*/...
    if [[ -d "$dir/runs" ]]; then
        runs_dir="$dir/runs"
    else
        runs_dir="$dir"
    ...
    # plot_omp_stage_scaling.py: stdout stage tables + results/*.csv + *.png
    #   (Stage 1: sum of PTimerWall sub-phases RNG+Z-loop+Mirror+FFT, not TIMING SUMMARY generation line;
    #    Stage 3 streaming: Unpack/1D-Y-FFT/I/O from log Stage 4 max-rank lines)
    python3 "$plot_script" \
        --runs-dir "$runs_dir" \
        --results "$results_dir"
}
