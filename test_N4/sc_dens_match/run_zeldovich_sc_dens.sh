#!/bin/bash
# Script to run zeldovich-PLT with PLT disabled for density comparison (ZD_qdensity=2)
...
PARAM_FILE="/home/helenshao/InitialConditions/hermitian_3d_matrix_production/test_N4/sc_dens_match/param_N4_sc_dens.par"
OUTPUT_DIR="${ZELDOVICH_DIR}/output_N4_sc_dens"
RNG_LOG_DIR="/home/helenshao/InitialConditions/hermitian_3d_matrix_production/test_N4/sc_dens_match/rng_logs"

echo "=========================================="
echo "Running zeldovich-PLT with PLT DISABLED (self-conjugate density comparison)"
echo "Using ZD_qdensity=2 (density only, no displacements)"
...
if ! grep -q "ZD_qdensity = 2" "$PARAM_FILE"; then
    ...
    echo "This script expects ZD_qdensity=2 (density only mode). Please check the parameter file."
    exit 1
fi
...
echo ""
echo "=========================================="
echo "zeldovich-PLT (self-conjugate density comparison) completed"
echo "Using ZD_qdensity=2 (density only, no displacements)"
