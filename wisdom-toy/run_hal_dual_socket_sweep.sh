#!/usr/bin/env bash
# HAL dual-socket OMP scaling sweep for fftw_2d_thread_sweep.
...
# You can override these at launch, e.g. N=4096 REPEATS=20 ./run_hal_dual_socket_sweep.sh
N="${N:-2048}"
NARRAY="${NARRAY:-4}"
REPEATS="${REPEATS:-30}"
THREADS=(${THREADS:-16 20 24 28})

RUN_ROOT="${SCRIPT_DIR}/hal-dual-socket-sweep"
AFFINITY="0-27"
NUMAPIN=(numactl --cpunodebind=0,1 --membind=0,1)
...
for T in "${THREADS[@]}"; do
    ...
    {
        echo "=== FFTW 2D dual-socket sweep ==="
        echo "Date: $(date -Is)"
        echo "Host: $(hostname)"
