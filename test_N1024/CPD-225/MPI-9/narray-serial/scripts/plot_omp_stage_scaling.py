#!/usr/bin/env python3
"""
Parse MPI-9 OMP sweep timing summaries and generate scaling plots.

Conventions used here:
- Ignore old "Stage 2 (Metadata exchange)" because it is always zero here.
- Rename old Stage 3 -> new Stage 2
- Rename old Stage 4 -> new Stage 3

Outputs:
- results/mpi9_omp_stage_times.png
- results/mpi9_omp_stage_histogram.png
- results/mpi9_omp_stage_speedup.png
- results/mpi9_omp_stage_efficiency.png
- results/mpi9_omp_stage_summary.csv
...
def parse_log(path: Path) -> dict[str, float]:
    ...
    return {
        ...
        "Stage 2": float(stage2.group(1)),  # old Stage 3
        "Stage 3": float(stage3.group(1)),  # old Stage 4
...
def resolve_run_log(omp_dir: Path, threads: int) -> Path | None:
    expected = omp_dir / f"N1024_CPD225_MPI9_OMP{threads}_bin_generation.log"
    if expected.is_file():
        return expected
    ...
    for candidate in candidates:
        if f"OMP{threads}" in candidate.name:
            return candidate
...
def discover_logs(runs_dir: Path) -> list[tuple[int, Path]]:
    ...
    for omp_dir in sorted(runs_dir.glob("omp-*")):
        ...
        run_log = resolve_run_log(omp_dir, threads)
        if run_log is not None:
            logs.append((threads, run_log))
...
def make_plots(rows: list[tuple[int, dict[str, float]]], results_dir: Path) -> None:
    ...
    baseline_threads = rows[0][0]
    baseline = rows[0][1]
    threads = [threads for threads, _ in rows]
    ...
    for label in labels:
        ...
        ax.plot(threads, values, marker="o", linewidth=2, markersize=7, label=label, color=colors[label])
    ax.set_xlabel("OMP threads")
    ax.set_ylabel("Time (s)")
    ax.set_title("MPI-9 OMP Scaling: Stage Times")
    ax.set_xticks(threads)
    ax.grid(alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(results_dir / "mpi9_omp_stage_times.png", dpi=180)
    plt.close(fig)
    ...
    ax.set_xlabel("OMP threads")
    ax.set_ylabel("Time (s)")
    ax.set_title("MPI-9 OMP Scaling: Stage-Time Histogram")
    ax.set_xticks(x)
    ax.set_xticklabels(threads)
    ax.grid(axis="y", alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(results_dir / "mpi9_omp_stage_histogram.png", dpi=180)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(8, 5))
    for label in labels:
        speedup = [baseline[label] / data[label] for _, data in rows]
        ax.plot(threads, speedup, marker="o", linewidth=2, markersize=7, label=label, color=colors[label])
    ideal = [thread_count / baseline_threads for thread_count in threads]
    ax.plot(threads, ideal, "--", color="gray", linewidth=1.5, label="Ideal")
    ax.set_xlabel("OMP threads")
    ax.set_ylabel(f"Speedup vs OMP={baseline_threads}")
    ax.set_title("MPI-9 OMP Scaling: Stage Speedup")
    ax.set_xticks(threads)
    ax.grid(alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(results_dir / "mpi9_omp_stage_speedup.png", dpi=180)
    plt.close(fig)
    ...
    for label in labels:
        ...
        ax.plot(threads, efficiency, marker="o", linewidth=2, markersize=7, label=label, color=colors[label])
    ax.axhline(100.0, linestyle="--", color="gray", linewidth=1.5, label="Ideal")
    ax.set_xlabel("OMP threads")
    ax.set_ylabel(f"Efficiency vs OMP={baseline_threads} (%)")
    ax.set_title("MPI-9 OMP Scaling: Stage Efficiency")
    ax.set_xticks(threads)
    ax.set_ylim(bottom=0)
    ax.grid(alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(results_dir / "mpi9_omp_stage_efficiency.png", dpi=180)
    plt.close(fig)
...
def main() -> int:
    ...
    print_summary(rows)
    results_dir.mkdir(parents=True, exist_ok=True)
    write_csv(rows, results_dir / "mpi9_omp_stage_summary.csv")
