...
Supports RVZelParticle format (32 bytes per particle).

Format breakdown (32 bytes total):
  Grid indices (8 bytes):
    i (uint16): Z-coordinate index
    j (uint16): Y-coordinate index
    k (uint16): X-coordinate index
    Padding (2 bytes) for 4-byte alignment
  
  Displacement vector (12 bytes, 3 floats):
    displ[0]: Z-displacement (from imag(slab2))
    displ[1]: Y-displacement (from real(slab2))
    displ[2]: X-displacement (from imag(slab1))
  
  Velocity vector (12 bytes, 3 floats):
    vel[0]: Z-velocity (from imag(slab4))
    vel[1]: Y-velocity (from real(slab4))
    vel[2]: X-velocity (from imag(slab3))
"""
...
def read_particles(filename, num_particles=3):
    ...
    # RVZelParticle format (32 bytes):
    #   Grid indices: i(uint16), j(uint16), k(uint16) = 6 bytes
    #   Padding: 2 bytes for 4-byte alignment
    #   Displacement: 3 floats = 12 bytes
    ...
    for i in range(show_count):
        ...
        # Unpack format: 'HHH H fff fff'
        #   HHH: i(uint16), j(uint16), k(uint16) - grid indices (Z, Y, X)
        #   H: padding (2 bytes for 4-byte alignment)
        #   fff: displ[0], displ[1], displ[2] - displacement vector (Z, Y, X)
        #   fff: vel[0], vel[1], vel[2] - velocity vector (Z, Y, X)
        try:
...
if __name__ == '__main__':
    ...
    print()
    print("  Particle |   i(Z) j(Y) k(X)  |     displ[Z]     displ[Y]     displ[X]     |     vel[Z]      vel[Y]      vel[X]")
    print("  ---------|-------------------|--------------------------------------------------|-----------------------------------")
